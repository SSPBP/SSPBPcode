...
import SSPBP.pbp as pbp

class PBP_net:

    def __init__(self, X_train, y_train, n_hidden, n_epochs = 40,
        normalize = False, biasfree=False):
        ...
        """
            ...
            @param biasfree     Whether to use a "bias-free" version where the
                                bias term in each node's calculation is removed.
        """
        ...
        if not normalize:
            self.mean_y_train = self.mean_y_train * 0.0
            self.std_y_train = self.std_y_train / self.std_y_train
        ...
        self.pbp_instance = \
            pbp.PBP(n_units_per_layer, self.mean_y_train, self.std_y_train, biasfree=biasfree)
    ...
    def predict(self, X_test):
        ...
        """
            ...
            @return m       The predictive mean for the test target variables' slabs.
            @return v       The predictive variance for the test target
                            variables' slabs.
            @return p       The predictive probability for the test target variables' slabs.
        ...
        m, v, p, v_noise = self.pbp_instance.get_predictive_mean_and_variance(X_test)

        # We are done!

        return m, v, p, v_noise
