...
import SSPBP.network as network

import SSPBP.prior as prior

class PBP:

    def __init__(self, layer_sizes, mean_y_train, std_y_train, biasfree=False):
        ...
        self.prior = prior.Prior(layer_sizes, var_targets, biasfree=biasfree)

        # We create the network

        params = self.prior.get_initial_params()
        self.network = network.Network(params[ 'm_w' ], params[ 'v_w' ],
            params[ 'a' ], params[ 'b' ], biasfree=biasfree)
    ...
    def get_predictive_mean_and_variance(self, X_test):
        ...
        for i in range(X_test.shape[ 0 ]):
            m, v, p = self.predict_probabilistic(X_test[ i, : ])
        ...
        return mean, variance, prob, v_noise
